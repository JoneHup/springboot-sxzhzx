...
; (function($) {
    $.fn.windstagball = function(options) {
		...
		mouseX = 100 - (warp.offset().left + warp.width() / 2);
		mouseY = 600 - (warp.offset().top + warp.height() / 2);
		mouseX /= 5;
		mouseY /= 5;
        ...
        function init() {
            ...
            items.each(function(i) {
				...
				itemPosList[i].cx = param.radius * Math.cos(theta) * Math.sin(phi) - (800*Math.random()-400);
                itemPosList[i].cy = param.radius * Math.sin(theta) * Math.sin(phi) + (600*Math.random()-300);
				...
				var posX = (parseInt(itemPosList[i].cx + warp.width() / 2 - itemPosList[i].width / 2));
				var posY = (parseInt(itemPosList[i].cy + warp.height() / 2 - itemPosList[i].height / 2));
				if(posY >= 800){
					posY = 850;
				}else if(posY <= 60){
					posY = 60;
